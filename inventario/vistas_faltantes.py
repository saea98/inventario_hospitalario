# Vistas faltantes para agregar al archivo views.py
...
from .models import (
    Lote, Producto, MovimientoInventario, AlertaCaducidad
)
...
def detalle_movimiento(request, pk):
    ...
    return render(request, 'inventario/movimientos/detalle.html', context)


# Vista adicional para estadísticas de movimientos
@login_required
def estadisticas_movimientos(request):
    """Vista para mostrar estadísticas de movimientos"""
    ...
    stats_por_tipo = movimientos_recientes.values('tipo_movimiento').annotate(
        total=Count('id'),
        cantidad_total=Sum('cantidad')
    ).order_by('-total')
    
    # Estadísticas por institución
    stats_por_institucion = movimientos_recientes.values(
        'lote__institucion__denominacion'
    ).annotate(
        total=Count('id'),
        cantidad_total=Sum('cantidad')
    ).order_by('-total')[:10]
    ...
    context = {
        ...
        'stats_por_institucion': stats_por_institucion,
        'movimientos_diarios': reversed(movimientos_diarios),
        'total_movimientos_mes': movimientos_recientes.count(),
    }
    
    return render(request, 'inventario/movimientos/estadisticas.html', context)
