...
class ImprimirEPAView(LoginRequiredMixin, View):
    """Genera e imprime el documento EPA (Entrada de Existencias en Almacén) usando ReportLab"""
    ...
    def get(self, request, pk):
        ...
        # Intentar usar ReportLab primero, si no está disponible usar WeasyPrint o template HTML
        try:
            ...
            use_reportlab = False
            # Intentar usar WeasyPrint como alternativa
            try:
                from weasyprint import HTML
                use_weasyprint = True
            except ImportError:
                use_weasyprint = False
                # Si ninguna está disponible, usar el template HTML como fallback
                context = {
                    'llegada': llegada,
        ...
        if not use_reportlab and use_weasyprint:
            ...
            html_string = render(request, 'inventario/llegadas/imprimir_epa.html', context).content.decode('utf-8')
            pdf = HTML(string=html_string, base_url=request.build_absolute_uri('/')).write_pdf()
            response = HttpResponse(pdf, content_type='application/pdf')
            response['Content-Disposition'] = f'inline; filename="EPA_{llegada.folio}.pdf"'
            return response
        ...
        doc = SimpleDocTemplate(
            ...
            leftMargin=0.5*inch,
            topMargin=1.3*inch,  # Espacio para el encabezado
            bottomMargin=1.2*inch  # Espacio para el pie de página
        )
        ...
        # Preparar datos para header y footer
        logo_path = os.path.join(settings.BASE_DIR, 'static', 'images', 'logo_epa.jpg')
        logo_footer_path = os.path.join(settings.BASE_DIR, 'static', 'images', 'logo_pie_pagina.png')
        
        # Funciones para header y footer
        def header_footer(canvas, doc):
            """Función que se ejecuta en cada página para dibujar header y footer"""
            # Guardar estado
            canvas.saveState()
            ...
            try:
                if os.path.exists(logo_path):
                    ...
                    logo.drawOn(canvas, 0.5*inch, letter[1] - 0.7*inch)
            except:
                pass
            
            # Texto del header
            canvas.setFont('Helvetica-Bold', 11)
            canvas.drawString(0.5*inch, letter[1] - 0.85*inch, 'DIRECCIÓN GENERAL')
            
            canvas.setFont('Helvetica', 9)
            canvas.drawString(0.5*inch, letter[1] - 1.0*inch, 'COORDINACIÓN ESTATAL EN LA CIUDAD DE MÉXICO IMSS BIENESTAR')
            canvas.drawString(0.5*inch, letter[1] - 1.1*inch, 'JEFATURA DE SERVICIOS ADMINISTRATIVOS Y FINANZAS')
            
            # Línea debajo del header
            canvas.setLineWidth(2)
            canvas.line(0.5*inch, letter[1] - 1.2*inch, letter[0] - 0.5*inch, letter[1] - 1.2*inch)
            
            # FOOTER - Pie de página fijo
            # Logo del footer
            try:
                if os.path.exists(logo_footer_path):
                    logo_footer = Image(logo_footer_path, width=6*inch, height=0.5*inch)
                    logo_footer.drawOn(canvas, (letter[0] - 6*inch) / 2, 0.4*inch)
            except:
                pass
            
            # Leyenda
            canvas.setFont('Helvetica-Bold', 7)
            leyenda = "ESTE DOCUMENTO AMPARA LA ENTREGA DE BIENES MUEBLES MEDIANTE REMISIÓN Y/O FACTURA, SIN CONSIDERAR LOS COSTOS UNITARIOS Y/O TOTALES"
            # Centrar texto
            text_width = canvas.stringWidth(leyenda, 'Helvetica-Bold', 7)
            x_leyenda = (letter[0] - text_width) / 2
            canvas.drawString(x_leyenda, 0.2*inch, leyenda)
            
            # Línea arriba del footer
            canvas.setLineWidth(2)
            canvas.line(0.5*inch, 0.35*inch, letter[0] - 0.5*inch, 0.35*inch)
            
            # Restaurar estado
            canvas.restoreState()
        
        # Crear PageTemplate con header y footer
        frame = Frame(
            doc.leftMargin,
            doc.bottomMargin,
            doc.width,
        ...
        template = PageTemplate(id='epa_template', frames=frame, onPage=header_footer)
        ...
        story.append(Paragraph('DOCUMENTO DE ENTRADA DE EXISTENCIAS EN ALMACÉN CENTRAL<br/>IMSS-BIENESTAR CIUDAD DE MÉXICO', title_style))
        ...
        info_data = [
            ['FOLIO', llegada.folio or ''],
            ['FECHA', llegada.fecha_llegada_real.strftime('%d/%m/%Y') if llegada.fecha_llegada_real else ''],
            ['FACTURA O REMISIÓN', llegada.remision or ''],
            ['ORDEN DE SUMINISTRO', 
             llegada.numero_orden_suministro or 
             (llegada.cita.numero_orden_suministro if llegada.cita and llegada.cita.numero_orden_suministro else 'No disponible')],
            ['CONTRATO', llegada.numero_contrato or ''],
            ['FUENTE DE FINANCIAMIENTO', 'IMSS-BIENESTAR'],
        ]
        ...
        for i in range(0, len(info_data), 2):
            row = []
            if i < len(info_data):
                row.append(Paragraph(f'<b>{info_data[i][0]}:</b>', info_style))
                row.append(Paragraph(info_data[i][1], info_style))
            else:
                row.append('')
                row.append('')
            
            if i + 1 < len(info_data):
                row.append(Paragraph(f'<b>{info_data[i+1][0]}:</b>', info_style))
                row.append(Paragraph(info_data[i+1][1], info_style))
            else:
                row.append('')
                row.append('')
        ...
        info_table = Table(info_table_data, colWidths=[1.5*inch, 3*inch, 1.5*inch, 3*inch])
        ...
        # Datos del Proveedor
        proveedor_data = [
            [Paragraph('<b>DATOS DE PROVEEDOR:</b>', info_style), 
             Paragraph(llegada.proveedor.razon_social if llegada.proveedor else 'No disponible', info_style)]
        ]
        proveedor_table = Table(proveedor_data, colWidths=[1.5*inch, 6.5*inch])
        ...
        # Cantidad Total
        cantidad_data = [
            [Paragraph('<b>CANTIDAD DE BIENES MUEBLES RECIBIDA:</b>', info_style), 
             Paragraph(str(total_items), info_style)]
        ]
        cantidad_table = Table(cantidad_data, colWidths=[2.5*inch, 5.5*inch])
        cantidad_table.setStyle(TableStyle([
            ('ALIGN', (0, 0), (0, 0), 'LEFT'),
            ('ALIGN', (1, 0), (1, 0), 'LEFT'),
            ('VALIGN', (0, 0), (-1, -1), 'BOTTOM'),
        ]))
        story.append(cantidad_table)
        story.append(Spacer(1, 0.15*inch))
        ...
        obs_table_data = [
            [Paragraph('<b>OBSERVACIONES</b>', obs_title_style)],
            [Paragraph('LA REMISIÓN NO ESTA COSTEADA, POR LO QUE, SE CONSIDERAN LOS PRECIOS UNITARIOS DE LA PLATAFORMA FEDERAL, HASTA QUE SE REALICE LA CONCILIACIÓN CON LA UNIDAD ADMINISTRATIVA FEDERAL', obs_style)]
        ]
        obs_table = Table(obs_table_data, colWidths=[8*inch])
        ...
        firma_data = [
            ['ALMACENISTA', 'ENCARGADA DEL ÁREA DE ENTRADA', 'TITULAR DE LA OFICINA DE ENTRADA'],
            ['_____________________', '_____________________', '_____________________']
        ]
        firma_table = Table(firma_data, colWidths=[2.5*inch, 2.5*inch, 3*inch])
        ...
        # Tabla de Items
        table_data = [['PARTIDA', 'CLAVE', 'DESCRIPCIÓN', 'UNIDAD DE MEDIDA', 'CANTIDAD RECIBIDA', 'LOTE', 'CADUCIDAD']]
        
        for idx, item in enumerate(items, 1):
            # Obtener clave correcta
            clave = item.clave
            if not clave or clave == "DEFAULT" or clave.upper() == "DEFAULT":
                if item.producto and item.producto.clave_cnis:
            ...
            row = [
                str(idx),
                clave,
                item.producto.descripcion if item.producto else 'No disponible',
                'Pieza',
                str(item.cantidad_recibida),
                item.numero_lote or '',
                item.fecha_caducidad.strftime('%d/%m/%Y') if item.fecha_caducidad else ''
            ]
            table_data.append(row)
        
        items_table = Table(table_data, colWidths=[0.5*inch, 1*inch, 3*inch, 0.8*inch, 0.8*inch, 0.8*inch, 0.9*inch])
